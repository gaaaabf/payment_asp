	...
	*/
	...
	//	return $form;
		...
		// Concatenate each value
				 $first_data 	= 	$postdata['first_data'];
				 $order_details =   $postdata['order_details'];	
			 	 $second_data	=	$postdata['second_data'];
 				
 				 $result_order_details = [];
 				 $result_first_data = [];
 				 $result_second_data = [];	

 				 foreach ($first_data as $key => $value) {
					 $result_first_data += [$key => $value]; 
				}
				
				foreach ($order_details as $key => $value) {
					 $result_order_details += [$key => $value]; 
				}

				foreach ($second_data as $key => $value) {
					 $result_second_data += [$key => $value]; 
				}

	$order_details_new = [];
	$order_details_new[0] = $result_order_details;
	$order_details_new[1] = $result_order_details;

	$result_first_data["cancel_url"] = $form['#cancel_url'];
	$result_first_data["error_url"] = $form['#cancel_url'];

			$postdata = $result_first_data +  $result_second_data  ;	
	...
	$postdata_freecsv = $postdata['free_csv'];

		
	
	// Adding hashkey to parameters to be passed
	$result_order_details_new = [];
	foreach ($order_details as $key => $value) {
					 $result_order_details_new += [$key => $value."new"]; 
			}


			$postdata = $result_first_data   +  $result_second_data ; //array($result_order_details,$result_order_details_new)

	$postdata['free_csv'] = $postdata_freecsv;
