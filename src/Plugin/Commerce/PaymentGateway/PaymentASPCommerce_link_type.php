...
class PaymentASPCommerce_link_type extends OffsitePaymentGatewayBase implements SupportsNotificationsInterface {
	...
	*/
	public function onNotify(Request $request) {
	 ...
	 $order_id = str_replace("zenss", "", $request->get('order_id'));
...
// ksm($kana_familyName);
	...
	switch ($this->configuration['method_type']) {
			case 'webcvs':
				$free_csv = "LAST_NAME=".$kana_givenName.",FIRST_NAME=".$kana_familyName.",MAIL=" . $order->getEmail(). ",TEL=" . $payment_gateway_parameter . ",BILL_DATE=" . (int)$BILL_DATE;		
				break;
			case 'credit3d':
		...
		$postdata = [
			"first_data"		=> [
					...
					"sps_cust_no"		=> "",
					"sps_payment_no"	=> "",
					"order_id"			=> $orderData['order_id']."zenss",
